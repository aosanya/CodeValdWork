...
| FEAT-20260604-001 | Unified planner — `TaskStatusSplit` constant + CanTransitionTo rules; `parent_task_id` on Task schema; `task.plan.split` consumer that creates child Tasks and transitions parent → SPLIT; `maybeCompleteSplitParent` roll-up logic | 🚀 In Progress | — |
