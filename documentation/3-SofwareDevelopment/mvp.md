...
| FEAT-20260602-008 | Mid-flight `WorkflowRun` cancel — `POST /workflow-runs/{id}/cancel` flips `in_progress → cancelling → cancelled`; cascades `work.task.cancelled` per non-terminal task; quiesce signal lets AI/Functions cancel in-flight work | 🚀 In Progress | ~~FEAT-20260602-001~~ ✅, ~~FEAT-20260602-003~~ ✅ |
