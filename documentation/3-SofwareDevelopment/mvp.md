...
| FEAT-20260604-001 | Unified planner — `TaskStatusSplit` constant + CanTransitionTo rules; `parent_task_id` on Task schema; `task.plan.split` consumer that creates child Tasks and transitions parent → SPLIT; `maybeCompleteSplitParent` roll-up logic | ✅ Done | — |
...
| FEAT-20260605-001 | Schema v4 — `Deliverable` + `AcceptanceCriteria` entity types; `has_deliverable` + `has_acceptance_criteria` edges on Task + TaskTodo; bump schema to v4 | 📋 Not Started | — |
